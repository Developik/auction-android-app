...
public class ProductTest {
	...
	public void setUp() {
		...
		pictures = new ArrayList<>(Arrays.asList("1.png", "2.png"));
		startingBid = 5.0;
		currentBid = 5.0;
		sold = false;
		category = categories.get(2);
		try{
			product = new Product(name, date, pictures, startingBid, currentBid, start, end, sold, category);
		}
		catch (Exception e){
	...
	public void testProductCreatedCorrectly() {
		...
		ArrayList<String> testPictures = product.getPictures();
		for (int i=0; i < pictures.size(); i++){
			assertEquals(testPictures.get(i), pictures.get(i));
		}
